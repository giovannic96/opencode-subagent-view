...
2. Install its dependencies:
   ...
   This also runs a `prepare` script that builds `dist/*.js` from `src/*.ts(x)` (see [Why this plugin prebuilds its own JSX](#why-this-plugin-prebuilds-its-own-jsx) below).
...
- `dist/*.js`: build output, generated by `npm run build` from the matching file in `src/`, gitignored. Not committed, but shipped in the published npm package alongside `src/` (see [Why this plugin prebuilds its own JSX](#why-this-plugin-prebuilds-its-own-jsx)).
...
- `npm run build` builds `dist/*.js` from `src/*.ts(x)` (see [Why this plugin prebuilds its own JSX](#why-this-plugin-prebuilds-its-own-jsx)). This runs automatically via the `prepare` script whenever you `npm install` this repo from source, and the test suite depends on its output existing.
...
### Why this plugin prebuilds its own JSX

`src/plugin.tsx` uses JSX, and its type of JSX (`@opentui/solid`'s, not React's) is only chosen correctly when the host transpiling it knows to use `jsxImportSource: "@opentui/solid"`, normally configured through this project's own `tsconfig.json`.

That works fine when this plugin is loaded from an absolute path outside any `node_modules` folder, which is how earlier verification of this plugin (including the solid-js fix above) was done: a real `tsconfig.json` sits right there next to `src/tui.tsx`, and Bun finds it. It does **not** work for how this plugin is actually installed by a real npm user, whether through a plain `npm install` into some project or through opencode's own **"Install Plugin"** command: either way, this plugin's files end up inside a `node_modules` folder, and Bun does not apply a `tsconfig.json` to files resolved from inside `node_modules`, even one shipped as part of the very same package. Shipping `tsconfig.json` in the published package was tried and confirmed not to help for this exact reason. Without a `jsxImportSource` to go by, Bun falls back to assuming React's JSX runtime, and fails outright with `Cannot find module 'react/jsx-dev-runtime'`, since this plugin has no dependency on React at all.

The fix is to remove the ambiguity at build time instead of hoping the host resolves it correctly at load time. `npm run build` runs `bun build --no-bundle` against each file in `src/` that needs it, which transpiles JSX syntax into explicit calls with an explicit import (`import { jsxDEV } from "@opentui/solid/jsx-dev-runtime"`), while leaving every other import, `solid-js`, `@opentui/solid`, and this plugin's own sibling files, completely untouched and unbundled. The output in `dist/` is what `src/tui.tsx` actually dynamically imports at runtime; there is no more JSX syntax left in it for a host to make a wrong decision about, and the solid-js patch and the unbundled-dependency behavior described above are both unaffected, since nothing is inlined or resolved early.

This was verified with the same rigor as the solid-js fix above: a real npm tarball, installed with `--ignore-scripts` into a fresh nested project (mirroring exactly how opencode's own installer behaves), successfully imports and initializes, and a real opencode instance using opencode's own "Install Plugin" command against the actual published package renders the "Subagents" section correctly with live activity, starting from a completely clean state with no prior installation of this plugin at all.
