...
### Why this plugin pins its own JSX import source

`src/plugin.tsx` uses JSX, and its type of JSX (`@opentui/solid`'s, not React's) is only chosen correctly if the host transpiling it knows to use `jsxImportSource: "@opentui/solid"`. This project's own `tsconfig.json` configures that, and it's enough when this plugin is loaded from an absolute path outside any `node_modules` folder, since Bun finds a project's `tsconfig.json` normally in that case.

It is not enough for how this plugin is actually installed by a real npm user, whether through a plain `npm install` into some project or through opencode's own **"Install Plugin"** command: either way, this plugin's files end up inside a `node_modules` folder, and Bun does not apply a `tsconfig.json` to files resolved from inside `node_modules`, even one shipped as part of the very same package. Shipping `tsconfig.json` in the published package was tried and confirmed not to help for this exact reason. Without a `jsxImportSource` to go by in that case, Bun falls back to assuming React's JSX runtime, and fails outright with `Cannot find module 'react/jsx-dev-runtime'`, since this plugin has no dependency on React at all.

Prebuilding this plugin's JSX ahead of time (transpiling `src/plugin.tsx`'s JSX into explicit calls at publish time, still without bundling `solid-js` or `@opentui/solid` themselves) was also tried, and while it fixed the JSX resolution problem on its own, it broke rendering the same way the earlier bundling attempt did: `getOrCreateChildSessions` and the plugin registered and imported correctly, but the "Subagents" section still didn't render, verified live against a real opencode instance starting from a genuinely fresh `solid-js`. That was set aside too.

The fix that actually works is a per-file `@jsxImportSource` pragma comment at the very top of `src/plugin.tsx`: `/** @jsxImportSource @opentui/solid */`. This is read directly from the file itself by whatever transpiles it, rather than looked up externally, so it applies correctly no matter where the file is resolved from, including deep inside a `node_modules` folder. This project's own `tsconfig.json` is still kept, since it's what makes `tsc --noEmit` and editor tooling apply the same `jsxImportSource` during local development and type-checking, the pragma comment only covers what Bun does when it transpiles the file at load time.

This was verified with the same rigor as the solid-js fix above: a real npm tarball, installed with `--ignore-scripts` into a fresh nested project (mirroring exactly how opencode's own installer behaves), successfully imports and initializes, and a real opencode instance, loaded from an absolute path outside `node_modules`, renders the "Subagents" section correctly with live activity from a genuinely fresh `solid-js`, starting from a completely clean state.
